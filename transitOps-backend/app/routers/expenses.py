...
def list_expenses(skip: int = 0, limit: int = 20, db: Session = Depends(get_db), current_user: User = Depends(read_role)):
...
def create_expense(expense_in: ExpenseCreate, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
...
def update_expense(id: int, expense_in: ExpenseUpdate, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
...
def delete_expense(id: int, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
