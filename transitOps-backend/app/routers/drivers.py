...
def list_drivers(skip: int = 0, limit: int = 20, db: Session = Depends(get_db), current_user: User = Depends(read_role)):
...
def list_available_drivers(skip: int = 0, limit: int = 20, db: Session = Depends(get_db), current_user: User = Depends(read_role)):
...
def get_driver(id: int, db: Session = Depends(get_db), current_user: User = Depends(read_role)):
...
def create_driver(driver_in: DriverCreate, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
...
def update_driver(id: int, driver_in: DriverUpdate, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
...
def delete_driver(id: int, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
