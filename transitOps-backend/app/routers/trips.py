...
def list_trips(skip: int = 0, limit: int = 20, db: Session = Depends(read_role), current_user: User = Depends(read_role)):
...
def get_trip(id: int, db: Session = Depends(read_role), current_user: User = Depends(read_role)):
...
def create_trip(trip_in: TripCreate, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
...
def update_trip(id: int, trip_in: TripUpdate, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
...
def delete_trip(id: int, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
...
def dispatch_trip(id: int, dispatch_in: TripDispatch, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
...
def complete_trip(id: int, complete_in: TripComplete, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
...
def cancel_trip(id: int, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
