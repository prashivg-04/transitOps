...
def list_maintenance(skip: int = 0, limit: int = 20, db: Session = Depends(read_role), current_user: User = Depends(read_role)):
...
def create_maintenance(maintenance_in: MaintenanceCreate, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
...
def update_maintenance(id: int, maintenance_in: MaintenanceUpdate, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
...
def delete_maintenance(id: int, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
...
def close_maintenance(id: int, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
