...
def list_vehicles(skip: int = 0, limit: int = 20, search: str = None, db: Session = Depends(get_db), current_user: User = Depends(read_role)):
...
def list_available_vehicles(skip: int = 0, limit: int = 20, db: Session = Depends(get_db), current_user: User = Depends(read_role)):
...
def list_vehicles_by_status(status_val: VehicleStatus, skip: int = 0, limit: int = 20, db: Session = Depends(get_db), current_user: User = Depends(read_role)):
...
def get_vehicle(id: int, db: Session = Depends(get_db), current_user: User = Depends(read_role)):
...
def create_vehicle(vehicle_in: VehicleCreate, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
...
def update_vehicle(id: int, vehicle_in: VehicleUpdate, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
...
def delete_vehicle(id: int, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
