...
def list_fuel_logs(skip: int = 0, limit: int = 20, db: Session = Depends(get_db), current_user: User = Depends(read_role)):
...
def create_fuel_log(fuel_in: FuelLogCreate, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
...
def update_fuel_log(id: int, fuel_in: FuelLogUpdate, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
...
def delete_fuel_log(id: int, db: Session = Depends(get_db), current_user: User = Depends(write_role)):
