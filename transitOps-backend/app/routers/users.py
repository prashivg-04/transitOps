...
def list_users(skip: int = 0, limit: int = 20, db: Session = Depends(read_role), current_user: User = Depends(read_role)):
...
def get_user(id: int, db: Session = Depends(read_role), current_user: User = Depends(read_role)):
...
def create_user(user_in: UserCreate, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
...
def update_user(id: int, user_in: UserUpdate, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
...
def delete_user(id: int, db: Session = Depends(write_role), current_user: User = Depends(write_role)):
