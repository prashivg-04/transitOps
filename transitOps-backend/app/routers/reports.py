...
def get_fuel_efficiency_report(db: Session = Depends(read_role), current_user: User = Depends(read_role)):
...
def get_fleet_utilization_report(db: Session = Depends(read_role), current_user: User = Depends(read_role)):
...
def get_vehicle_roi_report(vehicle_id: int, db: Session = Depends(read_role), current_user: User = Depends(read_role)):
...
def get_trip_summary_report(db: Session = Depends(read_role), current_user: User = Depends(read_role)):
...
def get_driver_summary_report(db: Session = Depends(read_role), current_user: User = Depends(read_role)):
